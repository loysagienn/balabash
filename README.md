...
Тредовая архитектура: система — множество параллельных ранов над одним
append-only логом событий в Postgres; каждый ран владеет тредом — контекстом
и границей видимости. Построена с нуля по дизайн-доку «Balabash v2 —
архитектура» (Notion; «v2» в названии дока — историческое: проект начинался
как balabash-v2 рядом с первой версией, которую затем заменил).
